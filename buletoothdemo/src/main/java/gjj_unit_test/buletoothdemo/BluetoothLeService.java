...
public class BluetoothLeService extends Service {
    ...
    public final static UUID UUID_HEART_RATE_MEASUREMENT       = UUID.fromString(SampleGattAttributes.HEART_RATE_MEASUREMENT);//这是心率的UUID
     ...
     * 扫描后的BluetoothDevice加入到列表中，这时列表中就会有设备，通过getName可以获取设备的蓝牙名字，getAddress获取设备的蓝牙地址。
     列表出来了之后，点击某个设备进行连接。
     注意这里的连接跟2.0的蓝牙的连接不一样，通过设备的connectGatt方法进行连接，连接完成后会获得一个BluetoothGatt的对象，
     这个对象中有连接的一些重要信息，切记要保存好。
     */

    //连接时有一个回调函数mGattCallback，这个函数中有很多设备的相关信息，比如设备的状态啊，设备中的通道哈，一些服务啊之类的。
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        @Override
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {//当连接状态发生变化
            String intentAction;
        ...
        @Override
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {//发现新服务
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.e(TAG, "发现新服务===");
                broadcastUpdate(ACTION_GATT_SERVICES_DISCOVERED);
            } else {
                Log.e(TAG, "onServicesDeiscovered received: 发现失败服务===" + status);
            }
        }
    ...
    private void broadcastUpdate(final String action,
        ...
        if (UUID_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {//如果是心率测量的设备
            ...
            if (data != null && data.length > 0) {
                ...
                Log.e("characteristic.getValue",new String(data) + "\n" + stringBuilder.toString());
    ...
    public void readCharacteristic(BluetoothGattCharacteristic characteristic) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothAdapter not initialized==读取失败");
            return;
        }
